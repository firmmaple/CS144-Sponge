...
void TCPSender::fill_window() {
    ...
    if (_window_size == 0) {
        ...
        // 不行，在回答这个问题前，首先我们得知道为什么在窗口大小为0时，需要将窗口大小视为为1。
        // 假设目前我方(A)收到了对方(B)的ack包，B的窗口大小为0，并且A先前发送的所有报文都被B确认了，
        // 那这时候A就被卡住了，因为所有报文都被B接受了，B不会继续发送ACK包，
        // 就算之后B的窗口更新了，不再是0，我们也永远不会知道B的最新窗口大小。
        // 所以A必须主动发送一个1字节的报文用于探测，尽管有可能会被B拒绝（B的窗口大小没有更新，还是0），
        // 但是B会继续向我们发送ack包，告知A其最新的窗口大小。
        // 另外，就算这个1字节的报文被拒绝，稍后A也会超时重传这个报文，所以没被B接受也没关系。
        //
        // 言归正传，如果使用end_seqno+=1，那么直到B发送ACK包告诉A“我收到了你的新数据，你有新数据被我确认了”，
        // 随后A更新_base_seqno前，像这样1字节的探测报文只能发送一次
        // 而end_seqno=_next_seqno+1则是，只要A接受到一个窗口为0的ACK报文，A就发送一个1字节的探测报文
        //
        // 那这会有什么问题呢，想象这样一种情况，在完成三次握手后，
        ...
        // 但是报文8完全是没有必要发送的，并且如果B返回的segment还是0，A还是会继续发送，非常不合理
        //
        // 使用end_seqno=_next_seqno+1会导致test120和132失败，不清楚具体原因，但似乎是会莫名奇妙的收到一个RST报文
        // 导致其中一方退出，而另一方不断重传报文，最后timeout。
        // 具体原因需要进一步调试分析，最好是把tcp_sponge_socket的代码看一下。
        //
        // 另外一点就是，使用end_seqno=_next_seqno+1会没发通过test17的测试，
        // 因为调用end_input_stream的时候应为窗口大小还是0，会发送一个本不应该发送的报文
    }
